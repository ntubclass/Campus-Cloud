...
def _ssh_exec(
    ...
    stdout_text = stdout_ch.read().decode(errors="replace")
    stderr_text = stderr_ch.read().decode(errors="replace")
...
def _ssh_exec_streaming(
    ...
    while True:
        if time.monotonic() - start > timeout:
            channel.close()
            raise RuntimeError(f"SSH 命令執行超時 ({timeout}s)")

        # 使用 select 等待資料，避免忙碌迴圈
