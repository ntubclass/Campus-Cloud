    ...
    with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
        for page in pdf.pages:
            ...
            for key in sorted(lines_by_key.keys()):
                words_in_line = sorted(lines_by_key[key], key=lambda ww: float(ww.get("x0", 0.0)))
                text_line = " ".join(str(ww.get("text", "")).strip() for ww in words_in_line if str(ww.get("text", "")).strip())
                if text_line:
